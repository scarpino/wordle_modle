...
#Data#
######
raw_site <- read_html("https://www.powerlanguage.co.uk/wordle/main.c1506a22.js") %>% html_text() #read in the raw JS that has the word dictionaries 

#a kind of lazy way to zero in on the dictionaries
split_site <- strsplit(x = raw_site, split = "[;]")[[1]]
find_dictionaries <- grep(pattern = "var La=", x = split_site)
dictionaries <- split_site[find_dictionaries]
split_dictionaries <- strsplit(x = dictionaries, split = "]")[[1]] 

#non goal words
find_non_goal_words <- grep(pattern = "Ta=", x = split_dictionaries)
non_goal_words_raw <- split_dictionaries[find_non_goal_words]
non_goal_words_combine <- gsub(pattern = ",Ta=[", replacement = "", non_goal_words_raw, fixed = TRUE)
...
#goal words
find_goal_words <- grep(pattern = "var La=", x = split_dictionaries)
goal_words_raw <- split_dictionaries[find_goal_words]
goal_words_combine <- gsub(pattern = "var La=[", replacement = "", goal_words_raw, fixed = TRUE)
...
if(save_files == TRUE){
  ...
  write.csv(goal_words, file = paste0("../data/",timestamp, "_goal_wordles.csv"), row.names = FALSE)
}